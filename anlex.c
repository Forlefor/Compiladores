	...
	while((c=fgetc(archivo))!=EOF)
	{
		...
		else if (isalpha(c))
		{
			...
			do{
				...
				i++;
				c=fgetc(archivo);
				if (i>=TAMLEX) //Se supera el tamaño del buffer
					break;
		...
		else if (isdigit(c))
		{
				...
				{
					switch(estado){
					case 0: //Secuencia de digitos . o e
						c=fgetc(archivo);
						if (isdigit(c))
						{
							id[++i]=c;
					...
					case 1://un punto, debe seguir un digito 
						...
						if (isdigit(c))
						{
							...
							{
								ungetc (c, archivo);
								sprintf(msg,"No se esperaba salto de linea");
								estado=-1;
							}	
							else 
		...
		{//un caracter o una cadena de caracteres
			...
			do{
				...
				{
					error("Se llego al fin de archivo sin comilla de cierre");
				}
				else if(c=='\n')
				{	ungetc(c,archivo);
					error("Se llego al fin de linea sin comilla de cierre"); 
					break;
				}
				else{
					id[i]=c;
